...
function issigncompatible(S::Matrix, v::Vector; freeindices::Vector{Int64} = Int[], NL = false)
    ...
    optimize!(model)
    return is_solved_and_feasible(model) ? true : false
end

function isconcordant_old(rn::ReactionSystem, atol = 1.0e-12)
    S = netstoichmat(rn)
    (n, r) = size(S)
...
function moveforward(signpattern::Vector{Int64}, fixedsigns::Vector{Int64}, n::Int64)
    ...
    return length(signpattern) == n ? (return signpattern) : (return push!(signpattern, 1))
end
